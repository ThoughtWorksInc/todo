...
scalaVersion in Global := "3.3.5"

lazy val js = project

val indexHtml = taskKey[File](
  "Generate an index.html that follows TodoMVC's Application Specification"
)

indexHtml := {
  val linkedJs =
    (scalaJSLinkedFile in js in Compile).value // .asInstanceOf[org.scalajs.core.tools.io.FileVirtualJSFile]
  val document = <html lang="en" data-framework="binding-scala">
    ...
    for {
      ...
      file.relativeTo(baseDirectory.value).get.toString
    }/>
  }
    </head>
    <body>
    ...
    linkedJs.data.relativeTo(baseDirectory.value).get.toString
  }></script>
